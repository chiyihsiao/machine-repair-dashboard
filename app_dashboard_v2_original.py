...
def render_case_card(row: pd.Series) -> None:
    ...
    with st.expander(title, expanded=False):
        ...
        st.caption(f"狀態：{status}　｜　風險：{risk}　｜　資料來源：{row.get('資料來源', '對方網站同步')}")
...
if check_password():
    ...
    with st.sidebar:
        ...
        selected_statuses = st.multiselect("案件狀態", statuses, default=statuses)
    ...
    # ---- KPI ----
    total = len(filtered)
    completed = int(filtered["實際完工日期"].fillna("").astype(str).str.strip().ne("").sum())
    pending = int(filtered["精確進度狀態"].isin(PENDING_STATUSES).sum())
    overdue = int(filtered["逾期天數_num"].gt(0).fillna(False).sum())
    avg_days = filtered["維修天數_num"].dropna().mean()
    k1, k2, k3, k4, k5 = st.columns(5)
    k1.metric("目前案件", f"{total} 件")
    k2.metric("待處理／進行中", f"{pending} 件")
    ...
    with st.expander("資料品質與欄位說明"):
        ...
        st.write("月份篩選預設使用實際完工月份；未完工案件會歸入「未完工」。維修天數為實際完工日減申請日，逾期天數為實際完工日減預計完成日。")
